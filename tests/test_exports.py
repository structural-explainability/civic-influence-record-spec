...
from civic_influence_record_spec.utils.path_utils import data_spec_path, repo_root
...
def test_scope_exclusions_export_is_non_empty() -> None:
    ...
    assert "domain vocabularies beyond the CIR record profile" in export.excluded


def test_scope_exclusions_include_expected_core_exclusions() -> None:
    """Scope exclusions include required Civic Influence Record exclusions."""
    root = repo_root()
    export = build_scope_exclusions_export(version="0.1.0", root=root)

    assert "domain vocabularies beyond the CIR record profile" in export.excluded
    assert "causation" in export.excluded
    assert "endorsement" in export.excluded
    assert "control" in export.excluded
    assert "recommendation or decision logic" in export.excluded
